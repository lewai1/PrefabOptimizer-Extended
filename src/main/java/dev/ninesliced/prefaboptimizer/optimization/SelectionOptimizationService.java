...
final class SelectionOptimizationService {
    ...
    boolean optimize(
        ...
        BuilderToolsPlugin.addToQueue(player, playerRef, (queueRef, state, componentAccessor) -> {
            ...
            if (selection == null || !selection.hasSelectionBounds()) {
                playerRef.sendMessage(Message.raw("PrefabOptimizer: select a region first."));
                return;
            }
            ...
            if (stats.removedBlocks() <= 0) {
                playerRef.sendMessage(Message.raw(
                    "PrefabOptimizer finished: removed 0/" + stats.processedBlocks()
            ...
            playerRef.sendMessage(Message.raw(
                "PrefabOptimizer finished: removed " + stats.removedBlocks() + "/" + stats.processedBlocks()
    ...
    private static Boolean invokeBuilderToolsSelectionCheck(
        ...
        try {
            ...
            playerRef.sendMessage(Message.raw("PrefabOptimizer: could not verify BuilderTools selection state (" + e.getClass().getSimpleName() + ")."));
            return false;
        }
    }
